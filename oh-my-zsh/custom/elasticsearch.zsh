...
alias leafpush='git push -f home $(current_branch); ssh home "cd /media/ramdisk; if ! test -e ${PWD##*/}; then git clone ~/Repo/${PWD##*/}; fi; cd ${PWD##*/}; git fetch origin; git checkout $(current_branch); git reset --hard origin/$(current_branch)"'
